...
class Model(nn.Module):
    ...
    def forward(self, x, time_lag, y_obs, c_mean, c_std):
        ...
        return (
            result[0],  # Q / hidden flow
            result[1],  # storage
            result[2],  # loss
            result[3],  # constrained loss
            result[4],  # bypass
            result[5],  # input gate
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate pretrained MCP model 1 only; no training.")
