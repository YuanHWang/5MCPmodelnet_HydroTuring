...
try:
    ...
    from MCPBRNN_lib_tools.MRZoo_Relaxed import (  # noqa: E402
        MCPBRNN_PETconstraint_MassRelax_Regular,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate pretrained MCP mass-relaxation model only; no training.")
...
def main():
    ...
    checkpoint = resolve_path(args.checkpoint, SCRIPT_DIR / "model_epoch3.pt")
    ...
    diagnostic_names = [
        "Qsim", "storage", "loss", "loss_constrained", "bypass",
        "gate_i", "gate_o", "gate_l", "gate_l_constrained", "gate_f",
        "h_nout", "obs_std", "gate_mr",
    ]
