...
from MCPBRNN_lib_tools.MCNZoo import (  # noqa: E402
    MCPBRNN_Generic_PETconstraint_MultiParallel_outputlossshare_MCA1_InputLinearLayer_LossGateUpdate,
)
...
class Model(nn.Module):
    def __init__(self, spin_len, train_time_len):
        ...
        self.MCPBRNNNode = (
            MCPBRNN_Generic_PETconstraint_MultiParallel_outputlossshare_MCA1_InputLinearLayer_LossGateUpdate(
                input_size=1, hidden_size=3, gate_dim=1, gate_dim_ucorr=1,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate pretrained MCP model 4 (MN3 distributed input) only; no training.")
...
def main():
    ...
    checkpoint = resolve_path(args.checkpoint, SCRIPT_DIR / "best_model_epoch2999.pt")
